  ...
  return (
    <main className='flex flex-col min-h-screen'>
      ...
      <section className='px-4 pt-6 pb-4'>
        <h2 className='mb-4 font-semibold text-tema-brown text-xs uppercase tracking-widest'>
          테마별 단어 공부
        </h2>
        <div className='gap-3 grid grid-cols-2'>
          ...
          {temas.map(({ tema, emoji, name, count }) => (
            <div
              key={tema}
              className='flex items-center gap-4 bg-card px-4 py-4 border border-border rounded-2xl'
            >
              {/* Icon */}
              ...
              <div className='flex flex-col flex-1 items-center'>
                ...
                <span className='mb-2 text-tema-brown-muted text-xs'>
                  {count}개 단어
                </span>
                <Link
                  href={`/voca/${tema}`}
